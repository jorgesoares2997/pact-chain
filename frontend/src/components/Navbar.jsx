  ...
  return (
    <nav className="border-b border-purple-900/40 px-4 py-3 flex items-center justify-between bg-[#0f0f1a]/80 backdrop-blur sticky top-0 z-50">
      ...
      <div className="flex items-center gap-3">
        <Link to="/create" className="text-sm text-slate-300 hover:text-white hidden sm:block">
          New Pact
        </Link>
        {address ? (
          <div className="flex items-center gap-2">
            ...
            <button onClick={disconnect} className="text-xs text-slate-500 hover:text-red-400">
              Disconnect
            </button>
          </div>
        ) : (
