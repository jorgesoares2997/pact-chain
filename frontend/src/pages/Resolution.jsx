...
export default function Resolution() {
  ...
  function submitNps(score) {
    setNps(score);
    setNpsSent(true);
    if (window.plausible) window.plausible("nps_submitted", { props: { score: String(score) } });
  }
  ...
  const totalStaked = ((pact.stake_amount * pact.max_participants) / 1e7).toFixed(2);
  const fee = (parseFloat(totalStaked) * 0.02).toFixed(2);
  const payout = (parseFloat(totalStaked) - parseFloat(fee)).toFixed(2);

  function shareResult() {
    const text = `🏆 ${pact.title}\nWinner: ${winner.slice(0, 8)}…\nPayout: ${payout} USDC\n\nCreated on PactChain — make commitments that stick ⛓\n${window.location.origin}/pact/${id}/result`;
  ...
  return (
    <main className="max-w-md mx-auto px-4 py-10 text-center">
      {pact.status === "refunded" ? (
        <>
          ...
          <p className="text-slate-400 text-sm mb-8">
            No unanimity was reached within 48 hours of the deadline. All participants have been refunded.
          </p>
        </>
      ) : (
        <>
          ...
          <div className="bg-[#1a1730] border border-purple-500/40 rounded-2xl p-6 mb-6 text-left">
            ...
            <div className="grid grid-cols-3 gap-3 text-center">
              <div>
                ...
                <div className="text-white font-semibold text-sm">{totalStaked} USDC</div>
              </div>
              <div>
      ...
      {!npsSent ? (
        <div className="bg-[#1a1730] border border-slate-700 rounded-2xl p-5 text-left">
        ...
        <div className="bg-green-900/20 border border-green-700/40 rounded-2xl p-4 text-green-400 text-sm">
          Thanks for the feedback! Score: {nps}/10
        </div>
      )}

      <Link to="/" className="block mt-6 text-slate-500 hover:text-slate-300 text-sm transition-colors">
        ← Back to home
      </Link>
    </main>
  );
